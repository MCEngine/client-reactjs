...
description: Entry point for MCEngine/client-reactjs — how the shared instruction set resolves, which tools fire on which trigger, and where this repository's own rules live.
---

# AGENTS.md

This repository is **`MCEngine/client-reactjs`**, published as `@mcengine/client-reactjs`:
the web panel for **MCPluginManager**. It is a React single-page application and it holds no
state of its own — every fact it renders comes from `@mcengine/server-expressjs` over HTTP.
People use it to sign in, administer a namespace or an organization, publish and version
product jars, mint the scoped tokens their Minecraft servers download with, and tell each
registered server which version of which plugin it should be running.
...
`agents_model_naming_convention` has no row: this panel stores no model identifier and has
no `model_name` column. Add the row in the same commit as the first one it gains.
